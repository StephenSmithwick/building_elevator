...
class TestElevator:
    def test_move_opens_door_on_target(self):
        elevator = Elevator(level=0, door=Door.SHUT)
    ...
    def test_move_closes_open_door(self):
        elevator = Elevator(level=0, door=Door.OPEN)
...
class TestRoute:
    def test_upward_elevator_is_after(self):
        route = Route(1, 2)

        assert route._is_after(1) == True

        assert route._is_after(2) == False

    def test_downward_elevator_is_after(self):
        route = Route(2, 1)

        assert route._is_after(2) == True

        assert route._is_after(1) == False
